    ...
    OpenIDAuthorizationCodeInterface
{
     ...
     */
    public function getBackupSql($dbName = 'oauth2_server_php')
    {
        // Drop any old backups and create a new backup of tables to be created
        $sql = "
        DROP DATABASE IF EXISTS {$dbName}_backup;
        CREATE DATABASE {$dbName}_backup;
        USE {$dbName}_backup;

        CREATE TABLE {$this->config['client_table']} AS SELECT * FROM {$dbName}.{$this->config['client_table']};
        CREATE TABLE {$this->config['access_token_table']} AS SELECT * FROM {$dbName}.{$this->config['access_token_table']};
        CREATE TABLE {$this->config['code_table']} AS SELECT * FROM {$dbName}.{$this->config['code_table']};
        CREATE TABLE {$this->config['refresh_token_table']} AS SELECT * FROM {$dbName}.{$this->config['refresh_token_table']};
        CREATE TABLE {$this->config['user_table']} AS SELECT * FROM {$dbName}.{$this->config['user_table']};
        CREATE TABLE {$this->config['scope_table']} AS SELECT * FROM {$dbName}.{$this->config['scope_table']};
        CREATE TABLE {$this->config['public_key_table']} AS SELECT * FROM {$dbName}.{$this->config['public_key_table']};
        ";
