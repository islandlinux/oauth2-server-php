...
class Bootstrap
{
    ...
    public function runPdoSql(\PDO $pdo)
    {
        ...
        $pdo->prepare($sql)->execute(array('testtoken', 'Some Client', date('c', strtotime('+1 hour')), null));
        $pdo->prepare($sql)->execute(array('accesstoken-openid-connect', 'Some Client', date('c', strtotime('+1 hour')), 'testuser'));

        $sql = 'INSERT INTO oauth_authorization_codes (authorization_code, client_id, expires) VALUES (?, ?, ?)';
        $pdo->prepare($sql)->execute(array('testcode', 'Some Client', date('c', strtotime('+1 hour'))));
